...
Here, FEM data are inverted using the laterally constrained 3D inversion approach. Just like in the previous exercise, every transmitter is associated with a distinct sounding location and a 1D model is recovered for every sounding location; see `EM1DFM package overview <http://em1dfm.readthedocs.io/en/latest/content/overview.html>`__. However constraints are now being added to the set of 1D models such that they must be sufficiently smooth in the horizontal. The set of recovered 1D models can ultimately be interpolated and visualized on a 3D mesh. The laterally constrained 3D inversion algorithm is a computationally fast way to invert frequency-domain EM data while taking into account both vertical and horizontal variability in the Earth's conductivity and magnetic susceptibility.

As part of this exercise, the user will:

    - Create EM1DFM inversion objects
    - Set relevant inversion parameters
    - Attempt to explain the set of field observations using a set of laterally constrained 1D conductivity models (susceptibility models can be included)
    - Interpret inversion results generated by the algorithm
...
Here, the set of FEM data are inverted using the laterally constrained 3D approach.
